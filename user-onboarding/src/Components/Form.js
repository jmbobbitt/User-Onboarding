...
function Form() {
    ...
    const formSchema = yup.object().shape({
        ...
        email: yup
            .string()
            .email()
            .required('must be an actual email, with @abc123.com'), 
        password: yup  
            .string()
            .required('please provide a vaild password'), 
    ...
    const formSubmit = e => {
        ...
        axios.post('https://reqres.in/api/users', formState).then(results => setUser([...user, results.data])).catch(err => console.log(err));
    };
